...
import time
import Tree
from Primal_warfarin import Primal
import logger
import getopt
import csv
import numpy as np
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "f:e:d:b:t:p:r:g:m:u:c:",
                                   ["training_file=", "test_file=", "depth=", "branching_limit=", "time_limit=",
                                    "pred=", "robust=", "data_group=", "ml=", "fairness_bound=", "protected_col="])
    ...
    data_path_dict = {}
    if robust:
        data_path_dict = {'Warfarin_seed1': ('/../data/Warfarin_v2/seed1/', '/../Results_Robust/Warfarin_v2/fairness/seed1/'),
                      'Warfarin_seed2': ('/../data/Warfarin_v2/seed2/', '/../Results_Robust/Warfarin_v2/fairness/seed2/'),
                      'Warfarin_seed3': ('/../data/Warfarin_v2/seed3/', '/../Results_Robust/Warfarin_v2/fairness/seed3/'),
                      'Warfarin_seed4': ('/../data/Warfarin_v2/seed4/', '/../Results_Robust/Warfarin_v2/fairness/seed4/'),
                      'Warfarin_seed5': ('/../data/Warfarin_v2/seed5/', '/../Results_Robust/Warfarin_v2/fairness/seed5/'),
                          'Athey_v1_500': ('/../data/Athey_v1/500/', '/../Results_Robust/Athey_v1/500/'),
                          'Athey_v2_4000': ('/../data/Athey_v2/4000/', '/../Results_Robust/Athey_v2/4000/')}
    else:
        data_path_dict = {'Warfarin_seed1': ('/../data/Warfarin_v2/seed1/', '/../Results_Direct/Warfarin_v2/fairness/seed1/'),
                      'Warfarin_seed2': ('/../data/Warfarin_v2/seed2/', '/../Results_Direct/Warfarin_v2/fairness/seed2/'),
                      'Warfarin_seed3': ('/../data/Warfarin_v2/seed3/', '/../Results_Direct/Warfarin_v2/fairness/seed3/'),
                      'Warfarin_seed4': ('/../data/Warfarin_v2/seed4/', '/../Results_Direct/Warfarin_v2/fairness/seed4/'),
                      'Warfarin_seed5': ('/../data/Warfarin_v2/seed5/', '/../Results_Direct/Warfarin_v2/fairness/seed5/'),
    ...
    data_group_ml_outcome_cols_dict = {'Warfarin_3000': [f'{ml}{k}' for k in range(3)]}
    ...
    out_put_name = training_file.split('.csv')[0] + '_' + approach_name + '_d_' + str(depth) + '_t_' + str(
        ...
        branching_limit) + '_pred_' + str(prob_type_pred) + f'_ml_{ml}_{protected_col}_' + str(fairness_bound)
    ...
    primal = Primal(data_train, features, protected_col, treatment_col, true_outcome_cols, outcome, regression, prob_t, robust,
                           tree, branching_limit, fairness_bound,
                           time_limit)
    ...
    protected_df = data_train[data_train[protected_col] == 0]
    protected_prime_df = data_train[data_train[protected_col] == 1]

    zeta = primal.model.getAttr("X", primal.zeta)

    treatments_set = data_train[treatment_col].unique()

    with open(out_put_path + out_put_name + '.p', 'wb') as f:
        pickle.dump(tree_structure, f)

    if robust:
        nonwhite = 1/len(protected_df) * (np.sum([zeta[i, n, k]*(protected_df.at[i, outcome] - protected_df.at[i, regression[int(k)]])/protected_df.at[i, prob_t] for i in protected_df.index for n in tree.Nodes + tree.Terminals for k in treatments_set if protected_df.at[i, treatment_col] == int(k)]) + \
            np.sum([zeta[i, n, k]*(protected_df.at[i, regression[int(k)]]) for i in protected_df.index for n in tree.Nodes + tree.Terminals for k in treatments_set]))
        white =  1/len(protected_prime_df) * (np.sum([zeta[i, n, k]*(protected_prime_df.at[i, outcome] - protected_prime_df.at[i, regression[int(k)]])/protected_prime_df.at[i, prob_t] for i in protected_prime_df.index for n in tree.Nodes + tree.Terminals for k in treatments_set if protected_prime_df.at[i, treatment_col] == int(k)]) + \
            np.sum([zeta[i, n, k]*(protected_prime_df.at[i, regression[int(k)]]) for i in protected_prime_df.index for n in tree.Nodes + tree.Terminals for k in treatments_set]))
    else:
        nonwhite = 1/len(protected_df) * (np.sum([zeta[i, n, k]*(protected_df.at[i, regression[int(k)]]) for i in protected_df.index for n in tree.Nodes + tree.Terminals for k in treatments_set]))
        white =  1/len(protected_prime_df) * (np.sum([zeta[i, n, k]*(protected_prime_df.at[i, regression[int(k)]]) for i in protected_prime_df.index for n in tree.Nodes + tree.Terminals for k in treatments_set]))
    ...
    with open(out_put_path + result_file, mode='a') as results:
        ...
        results_writer.writerow(
            [approach_name, training_file.split('.csv')[0], len(data_train),
             ...
             prob_type_pred, ml, protected_col, fairness_bound, nonwhite, white
             ])
