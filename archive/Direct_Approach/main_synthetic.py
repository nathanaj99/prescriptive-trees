...
import time
import Tree
from Primal_synthetic import Primal
import logger
import getopt
import csv
import numpy as np
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "f:e:d:b:t:p:r:g:m:u:",
                                   ["training_file=", "test_file=", "depth=", "branching_limit=", "time_limit=",
                                    "pred=", "robust=", "data_group=", "ml=", "budget_constraint="])
    ...
    for opt, arg in opts:
        if opt in ("-f", "--training_file"):
            ...
            ml = arg
        elif opt in ("-u", "--budget_constraint"):
            budget_constraint = float(arg)
    ...
    if robust:
        data_path_dict = {'Warfarin_seed1': ('/../data/Warfarin_v2/seed1/', '/../Results_Robust/Warfarin_v2/seed1/'),
                      ...
                      'Warfarin_seed5': ('/../data/Warfarin_v2/seed5/', '/../Results_Robust/Warfarin_v2/seed5/'),
                          'Athey_v1_500': ('/../data/Athey_v1/500/', f'/../Results_Robust/Athey_v1/'),
        ...
        data_path_dict = {'Warfarin_seed1': ('/../data/Warfarin_v2/seed1/', '/../Results_Direct/Warfarin_v2/seed1/'),
                      'Warfarin_seed2': ('/../data/Warfarin_v2/seed2/', '/../Results_Direct/Warfarin_v2/seed2/'),
                      ...
                      'Warfarin_seed5': ('/../data/Warfarin_v2/seed5/', '/../Results_Direct/Warfarin_v2/seed5/'),
                          'Athey_v1_500': ('/../data/Athey_v1/500/', f'/../Results_Direct/Athey_v1/'),
    ...
    out_put_name = training_file.split('.csv')[0] + '_' + approach_name + '_d_' + str(depth) + '_t_' + str(
        ...
        branching_limit) + '_pred_' + str(prob_type_pred) + '_ml_' + str(ml) + '_budget_' + str(budget_constraint)
    ...
    primal = Primal(data_train, features, treatment_col, true_outcome_cols, outcome, regression, prob_t, robust,
                           tree, branching_limit, budget_constraint,
                           time_limit)
