...
class Primal:
    def __init__(self, data, features, treatment_col, true_outcome_cols, outcome, regression, prob_t, robust, tree, branching_limit, budget_constraint,
                 time_limit):
        self.data = data
    ...
    def create_primal_problem(self):
        ...
        # ADD FAIRNESS/BUDGET CONSTRAINT
        if self.budget_constraint < 0.9:
            max_treated = self.budget_constraint * len(self.data)
            self.model.addConstr(
                quicksum(
                    self.zeta[i, n, 1] for i in self.datapoints for n in self.tree.Nodes + self.tree.Terminals) <= max_treated)
