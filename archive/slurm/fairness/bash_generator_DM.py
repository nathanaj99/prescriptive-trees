...
def generate(data_groups, depths, thresholds, preds, mls, protected_cols, fairness_bounds, array, slurm_file):
        ...
        S+="\n"
        S+="\n"
        command = 'python main_warfarin.py ' + '-f ' +'${train_data_list[$SLURM_ARRAY_TASK_ID]}' + ' -e ' +'${test_data_list[$SLURM_ARRAY_TASK_ID]}' + " -d " + '${depth_list[$SLURM_ARRAY_TASK_ID]}' + " -t " + str(time_limit) + " -b " + str(100)+ " -p " + '${pred_type_list[$SLURM_ARRAY_TASK_ID]}'+ " -r direct" + " -g " + '${data_group_list[$SLURM_ARRAY_TASK_ID]}' + " -m " + '${ml_list[$SLURM_ARRAY_TASK_ID]}' + " -u " + '${fairness_bounds_list[$SLURM_ARRAY_TASK_ID]}' + " -c " + '${protected_cols_list[$SLURM_ARRAY_TASK_ID]}'
        S+=command
        S+="\n"
...
def main():
    ...
    slurm_file = 'slurm_DM_' + 'Warfarinv2_r0.11_addl.sh'
    ...
    fairness_bounds = [0.01, 0.02, 0.03, 0.04]
    array = (len(samples) * len(depths) * len(thresholds) * len(preds) * len(mls) * len(data_groups) * len(protected_cols) * len(fairness_bounds)) - 1
    generate(data_groups, depths, thresholds, preds, mls, protected_cols, fairness_bounds, array, slurm_file)
