...
def generate(data_groups, depths, thresholds, preds, mls, protected_cols, fairness_bounds, array, slurm_file):
        ...
        S+="\n"
        S+="\n"
        command = 'python main_warfarin.py ' + '-f ' +'${train_data_list[$SLURM_ARRAY_TASK_ID]}' + ' -e ' +'${test_data_list[$SLURM_ARRAY_TASK_ID]}' + " -d " + '${depth_list[$SLURM_ARRAY_TASK_ID]}' + " -t " + str(time_limit) + " -b " + str(100)+ " -p " + '${pred_type_list[$SLURM_ARRAY_TASK_ID]}'+ " -r robust" + " -g " + '${data_group_list[$SLURM_ARRAY_TASK_ID]}' + " -m " + '${ml_list[$SLURM_ARRAY_TASK_ID]}' + " -u " + '${fairness_bounds_list[$SLURM_ARRAY_TASK_ID]}' + " -c " + '${protected_cols_list[$SLURM_ARRAY_TASK_ID]}'
        S+=command
        S+="\n"
...
def main():
    ...
    slurm_file = 'slurm_DR_' + 'Warfarinv2_seed4_addl.sh'
    depths = [2]
    thresholds = ["r0.06", "r0.11"]
    preds = ['tree']  #['true', 'tree', 'log'] ['tree']
    mls = ['lrrf', 'lr']
    protected_cols = ['White']
    fairness_bounds = [0.05, 0.06, 1.0]
