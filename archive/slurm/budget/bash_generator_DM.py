...
def generate(data_groups, depths, thresholds, preds, mls, budgets, array):
        ...
        slurm_file = f'slurm_DM_synthetic_1.0.sh'
        ...
        S+="\n"
        S+="\n"
        command = 'python main_synthetic.py ' + '-f ' +'${train_data_list[$SLURM_ARRAY_TASK_ID]}' + ' -e ' +'${test_data_list[$SLURM_ARRAY_TASK_ID]}' + " -d " + '${depth_list[$SLURM_ARRAY_TASK_ID]}' + " -t " + str(time_limit) + " -b " + str(100)+ " -p " + '${pred_type_list[$SLURM_ARRAY_TASK_ID]}'+ " -r direct" + " -g " + '${data_group_list[$SLURM_ARRAY_TASK_ID]}' + " -m " + '${ml_list[$SLURM_ARRAY_TASK_ID]}' + " -u " + '${budgets_list[$SLURM_ARRAY_TASK_ID]}'
        S+=command
        S+="\n"
