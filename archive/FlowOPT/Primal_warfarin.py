...
class Primal:
    def __init__(self, data, features, protected_col, treatment_col, true_outcome_cols, outcome, prob_t, tree, branching_limit,
                 fairness_bound, time_limit):
        self.data = data
    ...
    def create_primal_problem(self):
        ...
        if self.fairness_bound <= 0.6:
            self.model.addConstr((1/len(protected_df) * quicksum(self.z[i, 1]*(protected_df.at[i, self.outcome])/protected_df.at[i, self.prob_t] for i in protected_df.index)) -
                (1/len(protected_prime_df) * quicksum(self.z[i, 1]*(protected_prime_df.at[i, self.outcome])/protected_prime_df.at[i, self.prob_t] for i in protected_prime_df.index)) <= self.fairness_bound
                )

            self.model.addConstr((1/len(protected_prime_df) * quicksum(self.z[i, 1]*(protected_prime_df.at[i, self.outcome])/protected_prime_df.at[i, self.prob_t] for i in protected_prime_df.index)) -
                (1/len(protected_df) * quicksum(self.z[i, 1]*(protected_df.at[i, self.outcome])/protected_df.at[i, self.prob_t] for i in protected_df.index)) <= self.fairness_bound
                )
